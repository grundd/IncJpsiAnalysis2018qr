...
void PrepareMCTree(){
    ...
    // kCohJpsiToMu
    TFile *fFileInCoh = TFile::Open("Trees/AnalysisDataMC/AnalysisResults_MC_kCohJpsiToMu.root", "read");
    if(fFileInCoh) Printf("Input data loaded.");

    TTree *fTreeInCoh = dynamic_cast<TTree*> (fFileInCoh->Get("AnalysisOutput/fTreeJPsiMCRec"));
    if(fTreeInCoh) Printf("Input tree loaded.");

    // kIncohJpsiToMu
    TFile *fFileInInc = TFile::Open("Trees/AnalysisDataMC/AnalysisResults_MC_kIncohJpsiToMu.root", "read");
    if(fFileInInc) Printf("Input data loaded.");

    TTree *fTreeInInc = dynamic_cast<TTree*> (fFileInInc->Get("AnalysisOutput/fTreeJPsiMCRec"));
    if(fTreeInInc) Printf("Input tree loaded.");

    // Create new MC trees with applied cuts
    TFile fFileOut("Trees/InvMassFitMC/InvMassFitMC.root","RECREATE");
    ...
    ConnectTreeVariablesMCRec(fTreeInCoh);

    Printf("%lli entries found in the tree.", fTreeInCoh->GetEntries());
    ...
    ConnectTreeVariablesMCRec(fTreeInInc);

    Printf("%lli entries found in the tree.", fTreeInInc->GetEntries());
    nEntriesAnalysed = 0;
