...
void runAnalysis()
{
    ...
    AliAnalysisManager *mgr = new AliAnalysisManager("AliAnalysisTaskCentralJpsi_DG_electrons");
    // ESD input handler
    ...
    gInterpreter->LoadMacro("AliAnalysisTaskCentralJpsi_DG_electrons.cxx++g");
    char txt_cmd[120];
    sprintf(txt_cmd,"AddTaskCentralJpsi_DG_electrons.C()");
    AliAnalysisTaskCentralJpsi_DG_electrons *task = reinterpret_cast<AliAnalysisTaskCentralJpsi_DG_electrons*>(gInterpreter->ExecuteMacro(txt_cmd));
#else
    gROOT->LoadMacro("$ALICE_ROOT/ANALYSIS/macros/AddTaskPIDResponse.C");
    AddTaskPIDResponse();
    gROOT->LoadMacro("AliAnalysisTaskCentralJpsi_DG_electrons.cxx++g");
    gROOT->LoadMacro("AddTaskCentralJpsi_DG_electrons.C");
    AliAnalysisTaskCentralJpsi_DG_electrons *task = AddTaskCentralJpsi_DG_electrons();
#endif
    ...
    if(local) {
        ...
        if(!MC) chain->Add("/home/david/alice/IncJpsiAnalysis2018qr/Data/Data_000296623/101_AliESDs.root");
            ...
            if(iMCDataset == 7) chain->Add("/home/david/alice/IncJpsiAnalysis2018qr/Data/MC_kCohJpsiToElRad_295588_001/AliESDs.root"); 
        }
    ...
    // ================================================================
        ...
        alienHandler->SetAdditionalLibs("AliAnalysisTaskCentralJpsi_DG_electrons.cxx AliAnalysisTaskCentralJpsi_DG_electrons.h");
        alienHandler->SetAnalysisSource("AliAnalysisTaskCentralJpsi_DG_electrons.cxx");
        // select the aliphysics version. all other packages
        // are LOADED AUTOMATICALLY!
        alienHandler->SetAliPhysicsVersion("vAN-20220221_ROOT6-1");
        ...
        if(MC){
            ...
            if(iMCDataset == 7) alienHandler->SetGridDataDir("/alice/sim/2020/LHC20g1/kCohJpsiToEl");  
        ...
        alienHandler->SetMergeViaJDL(kFALSE); // set to kFALSE to download merged results
        ...
        if(gridTest) {
            ...
            alienHandler->SetRunMode("terminate"); // "full" or "terminate"
            mgr->StartAnalysis("grid");
        }
    }
    /*
