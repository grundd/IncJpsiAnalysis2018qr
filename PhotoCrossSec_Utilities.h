...
void ReadInputHSModel()
{
    ...
    ifstream ifs;
    ifs.open("PhotoCrossSec/HSModel/data-dtdy-y_0.6-Run1.txt");
...
void ReadInputGuzey()
{
    ...
    ifstream ifs;
    ifs.open("PhotoCrossSec/Guzey/incoh_tdep_nuc_run2.dat");
...
void ReadInputHeikki()
{
    // read the input file for Guzey's model predictions
    ifstream ifs;
    ifs.open("PhotoCrossSec/Heikki/ipsat_hight_alice_112021/no_photon_flux/incoherent_fluct");
    ...
    ifs.close();
    ifs.open("PhotoCrossSec/Heikki/ipsat_hight_alice_112021/no_photon_flux/incoherent_nofluct");
    for(Int_t i = 0; i < nData_HM; i++){
        ifs >> abs_t_HM[i];
        ifs >> sig_HM_noflu[i];
