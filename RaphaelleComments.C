...
void R02_AcceptanceDimuons(Bool_t pass3);
void R02_FillHistogram(TTree *t, TH1 *h, Bool_t MC, Bool_t pass3, Bool_t etaCut);
void R02_SetHistogram(TH1 *h);
void R15_DeltaPhiVsPt(Bool_t pass3);
...
void R02_AcceptanceDimuons(Bool_t pass3){
    ...
    TString str_f_data, str_t_data, str_f_MC, str_t_MC;
    if(!pass3){
        str_f_data = "Trees/AnalysisData_pass1/AnalysisResultsLHC18qrMerged.root";
        str_t_data = "AnalysisOutput/fTreeJPsi";
        str_f_MC = "Trees/AnalysisDataMC_pass1/AnalysisResults_MC_kIncohJpsiToMu.root";
        str_t_MC = "AnalysisOutput/fTreeJPsiMCRec";
    } else {
        str_f_data = "Trees/AnalysisData_pass3/AnalysisResults.root";
        str_t_data = "AnalysisOutput/fTreeJpsi";
        str_f_MC = "Trees/AnalysisDataMC_pass3/AnalysisResults_MC_kIncohJpsiToMu.root";
        str_t_MC = "AnalysisOutput/fTreeJpsi";
    }
    // open the data tree
    TFile *f_data = TFile::Open(str_f_data.Data(), "read");
    if(f_data) Printf("Input data loaded.");
    TTree *t_data = dynamic_cast<TTree*> (f_data->Get(str_t_data.Data()));
    if(t_data) Printf("Input tree loaded.");
    ConnectTreeVariables(t_data, pass3); 
    // open the MC tree
    TFile *f_MC = TFile::Open(str_f_MC.Data(), "read");
    if(f_MC) Printf("Input data loaded.");
    TTree *t_MC = dynamic_cast<TTree*> (f_MC->Get(str_t_MC.Data()));
    if(t_MC) Printf("Input tree loaded.");
    ConnectTreeVariablesMCRec(t_MC, pass3);     

    TH1D *h[4] = { NULL };
    TString titles[4] = {"h_data", "h_data_noEta", "h_MC", "h_MC_noEta"};
    for(Int_t i = 0; i < 4; i++){
        h[i] = new TH1D(titles[i].Data(), titles[i].Data(), 200, -1., 1.);
        h[i]->SetLineWidth(2);
    }

    R02_FillHistogram(t_data, h[0], kFALSE, pass3, kTRUE); // data
    R02_FillHistogram(t_data, h[1], kFALSE, pass3, kFALSE);// data without eta cut
    R02_FillHistogram(t_MC, h[2], kTRUE, pass3, kTRUE); // MC
    R02_FillHistogram(t_MC, h[3], kTRUE, pass3, kFALSE);// MC without eta cut
    ...
    TString folder = "Results/RaphaelleComments/R02_AccDimuons/";
    TString pass;
    if(!pass3)  pass = "pass1";
    else        pass = "pass3";

    c_eta->Print((folder + pass + "_etaCut.pdf").Data());
    c_eta->Print((folder + pass + "_etaCut.png").Data());
    c_noEta->Print((folder + pass + "_noEtaCut.pdf").Data());
    c_noEta->Print((folder + pass + "_noEtaCut.png").Data());
...
void R02_FillHistogram(TTree *t, TH1 *h, Bool_t MC, Bool_t pass3, Bool_t etaCut){
    ...
    for(Int_t iEntry = 0; iEntry < t->GetEntries(); iEntry++){
        ...
        // if MC:
            ...
            // pass3:
            // 0) fEvent non-empty
            // 1) nGoodTracksTPC == 2 &return kFALSEnue;
            if(pass3){
                // 3) At least two tracks associated with the vertex
                if(fVertexContrib < 2) continue;
                // 4) Distance from the IP lower than 15 cm
...
void R15_DeltaPhiVsPt(Bool_t pass3)
{
    ...
    TTree *t = dynamic_cast<TTree*> (f->Get(str_tree.Data()));
    if(t) Printf("MC rec tree loaded.");
    ConnectTreeVariablesMCRec(t, pass3);
    ...
    hProfiles[0]->SetLineColor(kBlue); // 0STG passed
    hProfiles[1]->SetLineColor(kCyan); // 0STG rejected
    hProfiles[2]->SetLineColor(kGreen);// 0OMU passed
    hProfiles[3]->SetLineColor(kRed);  // 0OMU rejected
    ...
    TString folder = "Results/RaphaelleComments/R15_DeltaPhiVsPt/";
    TString pass;
    if(!pass3)  pass = "pass1";
    else        pass = "pass3";
    ...
    for(Int_t i = 0; i < 4; i++){
        ...
        c[i]->Print((folder + pass + "_" + titles[i] + ".pdf").Data());
        c[i]->Print((folder + pass + "_" + titles[i] + ".png").Data());
    }
    ...
    c_profile->Print((folder + pass + "_" + "profiles.pdf").Data());
    c_profile->Print((folder + pass + "_" + "profiles.png").Data());
